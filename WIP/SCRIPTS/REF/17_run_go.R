...
run_go <- function(
  ...
  for (sig_file in list.files(path = input_sig, 
    ...
    go_plot <- 
      ...
      theme_minimal() +
      labs(
        title = paste("Top go terms ", cell_type, "_", comparison),
        x = "GO Term",
        y = "-log10(p-value)",
    ...
    if (save) {
      ...
      write.csv(go_df, file = file.path(go_dir, "csv",  paste0(cell_type, "_",  comparison, ".csv")))
      ggsave(filename = file.path(go_dir, "plot", paste0("GOPlot_", cell_type, "_",  comparison, ".png")), plot = go_plot, width = 12, height = 12, dpi = 300)
    }
    # CNET plot
    if (cnet) {
      ...
      if (nrow(top_terms) > 2) {
        cnet_plot <- enrichplot::cnetplot(
          ...
          circular = TRUE,
          node_label = 'gene') +
          ggtitle(paste("GO cnetplot ", cell_type, "_", comparison))
        
        ggsave(filename = file.path(cnet_dir, paste0("cnetplot_", cell_type, "_",  comparison, ".png")), plot = cnet_plot, width = 12, height = 12, dpi = 300)
      } else {
        message("Not enough terms for CnetPlot ", cell_type, '_', comparison)
      }
    }
      ...
      if (nrow(go_df) > 1 && length(unique(go_df$ID)) >1) {
        ...
        if (!is.null(similarity) && length(similarity) > 0) {
          ...
          if (!is.null(reduced) && length(reduced) > 0) {
            write.csv(reduced, file = file.path(rrvgo_dir, paste('rrvgo_', cell_type, "_", comparison, '.csv')))
            png(file = file.path(rrvgo_dir, paste('rrvgo_', cell_type, "_", comparison, '.png')), width = 12, height = 12, units = 'in', res = 300)
            dev.off()
              }
        } else {
          message("Not enough terms for rrvgo calculation ", cell_type, '_', comparison)
        }
      }
    } 
  }
