...
# For interactive DimPlot
# "~/SLC35A2_Olig2cKO_snRNA_JY/WIP/SCRIPTS/SourceCode/99_interactiveDimPlot.R"
# For FeaturePlot 
# "~/SLC35A2_Olig2cKO_snRNA_JY/WIP/SCRIPTS/SourceCode/99_shiny_featureplot.R"
# For Gene list used
...
# Final QC
```{r}
# '~/SLC35A2_Olig2cKO_snRNA_JY/WIP/SCRIPTS/SourceCode/99_QCPlotting.R'
```
...
Oligdp <- DimPlot(integrated_man_npc, label = T, repel = T) + NoLegend()
oligdpfeature <- FeaturePlot(integrated_man_npc, features = OligMarkers, label = T, repel = T)
...
# save
ggplot2::ggsave(Oligdp, file = file.path(plot_dir, "DimPlot_OligManSub.png"), width = 8, height = 8, dpi = 300)
ggplot2::ggsave(oligdpfeature, file = file.path(plot_dir, "FeaturePlot_OligManSub.png"), width = 32, height = 32, dpi = 300)
```
...
**-**: Zwilch (−0.77), Zgrf1 (−1.10)
Zic family transcription factor
differentiation/lineage commitment?

##### Some markers from paper.
```{r [V], echo=TRUE, message=TRUE, warning=TRUE, paged.print=TRUE}
# Read previous object if needed.
# integrated_man_npc <- qs::qread(file.path(qsave_dir, "OligoSubset", "10__man_npc.qs"))
papermarkers <- c("Npas1", "Clu", "Epas1", "Pnlip", "Ptprn", "Hes5", "Nrarp", "Notch", "Slc38a1")

Idents(integrated_man_npc) <- "CellTypes2"
fp <- FeaturePlot(integrated_man_npc, features = papermarkers, label = T, repel = T, reduction = 'umap')

# save
ggplot2::ggsave(fp, file = file.path(plot_dir, "FeaturePlot_OligManSub_papermarker.png"), width = 12, height = 12, dpi = 300)
```

## Slingshot for pseudotime analysis
```{r Slingshot echo=TRUE, message=TRUE, warning=TRUE, paged.print=TRUE}
# "~/SLC35A2_Olig2cKO_snRNA_JY/WIP/SCRIPTS/SourceCode/99_slingshotOPC.R"
# Read previous object if needed.
# integrated_man_npc <- qs::qread(file.path(qsave_dir, "OligoSubset", "10__man_npc.qs"))
...
umap2 <- Embeddings(integrated_man_npc, "umap")
celltype2 <- integrated_man_npc$CellTypes2

# Pseudotime Sligshot
lineages <- slingshot(umap, clusterLabels = celltype, reducedDim = "UMAP")
lineages2 <- slingshot(umap2, clusterLabels = celltype2, reducedDim = "UMAP")
...
umap_df2 <- as.data.frame(umap2)
colnames(umap_df2) <- c("umap_1", "umap_2")
umap_df2$cell_type <- celltype2
umap_df2 <- cbind(umap_df2, integrated_man_npc@meta.data)
...
umap_df2$pseudotime <-  lineages2@assays@data@listData[["pseudotime"]]
...
pseudo2 <- ggplot(umap_df2, aes(x = umap_1, y = umap_2, color = pseudotime)) +
  geom_point(size = 1) +
  scale_color_viridis_c(na.value = "grey50") +
  labs(title = "Pseudotime Trajectory (Slingshot)",
       color = "Pseudotime") +
...
condition <-ggplot(umap_df, aes(x = umap_1, y = umap_2, color = pseudotime)) +
  geom_point(size = 1) +
  scale_color_viridis_c(na.value = "grey50") +
  facet_wrap(~ TimeGeno) +  # adjust as needed
...
# A list of cell cycle markers, from Tirosh et al, 2015, is loaded with Seurat.  We can segregate this list into markers of G2/M phase and markers of S phase (From Seurat Vignetts)
s.genes <- cc.genes$s.genes
g2m.genes <- cc.genes$g2m.genes

# Case match
s.genes <- CaseMatch(search = s.genes, match = rownames(integrated_man_npc))
g2m.genes<- CaseMatch(search = g2m.genes, match = rownames(integrated_man_npc))

# Score
integrated_man_npc <- CellCycleScoring(integrated_man_npc, s.features = s.genes, g2m.features = g2m.genes, set.ident = TRUE)

# overlap gene
length(intersect(s.genes, rownames(integrated_man_npc)))
length(intersect(g2m.genes, rownames(integrated_man_npc)))

DimPlot(integrated_man_npc, label = T, group.by = "CellTypes2", split.by = "Phase", reduction = 'umap') 


#
integrated_test <- ScaleData(integrated_man_npc, vars.to.regress = c("S.Score", "G2M.Score")) 
integrated_test <- RunPCA(integrated_test, features = VariableFeatures(integrated_test)) 
integrated_test <- RunUMAP(integrated_test, dims = 1:30, reduction = 'pca')

DimPlot(integrated_test, label = T, group.by = "CellTypes2", reduction = 'umap') 

table <- integrated_test@meta.data %>%
  select(CellTypes2, Phase) %>%
  group_by(CellTypes2) %>% 
  count(Phase) %>% 
  pivot_longer(cols = c("G1", "G2M", "S"))

prop <- prop.table(table(integrated_test@meta.data$CellTypes2, integrated_test@meta.data$Phase), margin=1)
test <- chisq.test(table(integrated_test@meta.data$CellTypes2, integrated_test@meta.data$Phase))

DimPlot(integrated_test, group.by = "Phase",  label = T, repel = T)
```
