...
# save
qs::qsave(processed_list, file = file.path(qsave_dir, "04_processed_rna_list_dbl.qs"))

# Back transfer the metadata to integrated
...
for (i in seq_along(processed_list)) {
  individual_meta <- processed_list[[i]]@meta.data %>%
    tibble::rownames_to_column("CellID") %>%
    select(CellID, contains("DF.classifications"), contains("pANN")) # Select doublet-related columns
    
  all_individual_meta[[i]] <- individual_meta
}

# Combine all individual metadata
combined_individual_meta <- do.call(rbind, all_individual_meta)
