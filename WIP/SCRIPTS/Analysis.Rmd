...
# subset function
subset_by_types <- function(obj, meta_col, keep_levels) {
  sample <- obj[[]]
  if (!meta_col %in% colnames(sample)) {
    warning(sprintf("meta column '%s' not found; returning NULL", meta_col))
    return(NULL)
  }
  keep_cells <- rownames(sample)[sample[[meta_col]] %in% keep_levels]
  if (length(keep_cells) == 0) return(NULL) 
  subset(obj, cells = keep_cells)
}

# Cell clusters
oligos <- c("OPC", "COP", "NFOL", "MFOL", "Mature_OL")
oligos_npc <- c(oligos, "NPC")

# manual
processed_list_man <- lapply(processed_list, subset_by_types, meta_col = "CellTypes", keep_levels = oligos)
processed_list_man <- Filter(Negate(is.null), processed_list_man)

# ref
processed_list_ref <- lapply(processed_list, subset_by_types, meta_col = "ref_short", keep_levels = oligos)
processed_list_ref <- Filter(Negate(is.null), processed_list_ref)

# manual and NPC
processed_list_man_npc <- lapply(processed_list, subset_by_types, meta_col = "CellTypes", keep_levels = oligos_npc)
processed_list_man_npc <- Filter(Negate(is.null), processed_list_man_npc)

# ref and NPC
processed_list_ref_npc <- lapply(processed_list, subset_by_types, meta_col = "ref_short", keep_levels = oligos_npc)
processed_list_ref_npc <- Filter(Negate(is.null), processed_list_ref_npc)
...
# Read previous object if needed.
# integrated_ref <- qs::qread(file.path(qsave_dir, "09_ref_annotated_obj_ref.qs"))
# integrated_man <- qs::qread(file.path(qsave_dir, "09_ref_annotated_obj_man.qs"))
# integrated_ref_npc <- qs::qread(file.path(qsave_dir, "09_ref_annotated_obj_ref_npc.qs"))
# integrated_man_npc <- qs::qread(file.path(qsave_dir, "09_ref_annotated_obj_man_npc.qs"))
...
# Save
qs::qsave(integrated_ref, file = file.path(qsave_dir, "10_ref.qs"))
qs::qsave(integrated_man, file = file.path(qsave_dir, "10_man.qs"))
qs::qsave(integrated_ref_npc, file = file.path(qsave_dir, "10_ref_npc.qs"))
qs::qsave(integrated_man_npc, file = file.path(qsave_dir, "10_man_npc.qs"))
```

### Clean cluster name
```{r [O] Clean ref annotation, echo=TRUE, message=TRUE, warning=TRUE, paged.print=TRUE}
# Read previous object if needed.
# integrated_ref <- qs::qread(file.path(qsave_dir, "10_ref.qs"))
# integrated_man <- qs::qread(file.path(qsave_dir, "10_man.qs"))
# integrated_ref_npc <- qs::qread(file.path(qsave_dir, "10_ref_npc.qs"))
# integrated_man_npc <- qs::qread(file.path(qsave_dir, "10_man_npc.qs"))
...
sn <- c("ExCA3", "ExCortex", "MFOL", "Mature_OL", "OPC", "COP", "Interneuron", "NFOL", "Interneuron", "MGE", "Granule_DG", "ExCA1", "Interneuron", "Cajal_Retzius", "CGE", "NPC", "Interneuron", "Vascular", "Vascular", "Granuel_Blast_DG")
name_map <- setNames(sn, ln)
...
# Set marker genes to plot
markers <- c('Aqp4', 'Aldh1l1', 'Gfap', 'P2ry12', 'Aif1', 'Tmem212', 'Foxj1', 'Gad1','Gad2', 'Slc17a7', 'Nes', 'Pdgfra', 'Cspg4', 'Enpp6', 'Plp1', 'Mog', 'Vim')
...
# Create DotPlot
Seurat::DotPlot(integrated_ref_npc, features = markers, group.by = "ref_short2",cols = c("darkgrey", "indianred1")) +
  ggplot2::coord_flip() +
  ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 90, vjust = 0.8, hjust = 1, size = 10), axis.text.y = ggplot2::element_text(size = 10), axis.title.x = ggplot2::element_blank(), axis.title.y = ggplot2::element_blank(), legend.title = element_text(size = 10), legend.text = element_text(size = 8)) + ggtitle("ref based cluster subset") |
Seurat::DotPlot(integrated_man_npc, features = markers, group.by = "ref_short2",cols = c("darkgrey", "indianred1")) +
  ggplot2::coord_flip() +
  ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 90, vjust = 0.8, hjust = 1, size = 10), axis.text.y = ggplot2::element_text(size = 10), axis.title.x = ggplot2::element_blank(), axis.title.y = ggplot2::element_blank(), legend.title = element_text(size = 10), legend.text = element_text(size = 8)) + ggtitle("manual cluster subset")

# DimPlot
SeuratExtend::DimPlot2(integrated_ref_npc, reduction = 'umap', group.by = 'ref_short2', label = TRUE, label.size = 5, repel = TRUE, theme = list(Seurat::NoLegend(), Seurat::NoAxes(), ggtitle("ref based cluster subset"))) +  
  theme_umap_arrows(x_label = "UMAP_1",y_label = "UMAP_2") |
SeuratExtend::DimPlot2(integrated_man_npc, reduction = 'umap', group.by = 'ref_short2', label = TRUE, label.size = 5, repel = TRUE, theme = list(Seurat::NoLegend(), Seurat::NoAxes(), ggtitle("manual cluster subset"))) +  
  theme_umap_arrows(x_label = "UMAP_1",y_label = "UMAP_2")

SeuratExtend::DimPlot2(integrated_ref_npc, reduction = 'umap', group.by = 'CellTypes', label = TRUE, label.size = 5, repel = TRUE, theme = list(Seurat::NoLegend(), Seurat::NoAxes(), ggtitle("ref based cluster subset"))) +  
  theme_umap_arrows(x_label = "UMAP_1",y_label = "UMAP_2") |
SeuratExtend::DimPlot2(integrated_man_npc, reduction = 'umap', group.by = 'CellTypes', label = TRUE, label.size = 5, repel = TRUE, theme = list(Seurat::NoLegend(), Seurat::NoAxes(), ggtitle("manual cluster subset"))) +  
  theme_umap_arrows(x_label = "UMAP_1",y_label = "UMAP_2")


# FeaturePlot
SeuratExtend::FeaturePlot3.grid(integrated_ref_npc, features = OligMarkers)
SeuratExtend::FeaturePlot3.grid(integrated_ref_npc, features = OligMarkers_selected)
Seurat::FeaturePlot(integrated_ref_npc, features = OligMarkers, label = T, repel = T)
Seurat::FeaturePlot(integrated_ref_npc, features = OligMarkers_selected, label = T, repel = T)

SeuratExtend::FeaturePlot3.grid(integrated_man_npc, features = OligMarkers)
SeuratExtend::FeaturePlot3.grid(integrated_man_npc, features = OligMarkers_selected)
Seurat::FeaturePlot(integrated_man_npc, features = OligMarkers, label = T, repel = T)
...
Idents(integrated_man_npc) <- "CellTypes"
...
Idents(integrated_ref_npc) <- "CellTypes"
DimPlot(integrated_ref_npc, label = T, repel = T) + NoLegend()
FeaturePlot(integrated_ref_npc, features = OligMarkers_selected)
FeaturePlot(, features = markers)

DimPlot(integrated_man_npc, reduction = "umap", label = T, repel = T, split.by  = "TimePoint", group.by = "GenoType") |
DimPlot(integrated_man_npc, reduction = "pca", label = T, repel = T, group.by = "ref_short2") + NoLegend() |
DimPlot(integrated_man_npc, reduction = "umap", label = T, repel = T, group.by = "CellTypes") + NoLegend() + NoLegend() |
  DimPlot(integrated_ref_npc, reduction = "umap", label = T, repel = T, group.by = "CellTypes") + NoLegend() |
  DimPlot(integrated_ref_npc, reduction = "pca", label = T, repel = T, group.by = "ref_short2") + NoLegend() |
  DimPlot(integrated_ref_npc, reduction = "pca", label = T, repel = T, group.by = "CellTypes") + NoLegend() + NoLegend()
```
...
eigencorplot(p, metavars = c('CellTypes', 'ref_short2'))
pairsplot(p)
```
