  ...
  if (length(samples) != length(sobj_list)){
    stop("Sample length and objects list length are not the same. \nSample length: ", length(samples), "\nObject list length: ", length(sobj_list))
      ...
      for (sample in names(sobj_list)) {
        ...
        if (save) {qs::qsave(sobj, file = file.path(save_dir_qs, "INDIVIDUAL_SeuratObjects", paste0(sample, "_rna_filtered.qs")))
          message("Saved object: ", sample, " to ", file.path(save_dir_qs, "INDIVIDUAL_SeuratObjects", paste0(sample, "_rna_filtered.qs")), ".")
        }
      }

      if (save) {readr::write_csv(qc_summary, file = file.path(save_dir_qc, "QC_summary.csv"))
        message("QC summary saved to: ", file.path(save_dir_qc, file.path(sample, "_rna_filtered.qs")))
        }
