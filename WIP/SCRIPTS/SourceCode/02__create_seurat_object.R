...
# source("~/SLC35A2_Olig2cKO_snRNA/WIP/SCRIPTS/SourceCode/02_create_seurat_object.R")
...
#' `sample_filtered_barcodes.csv`. Optionally, each Seurat object can be saved to disk.
#'
...
create_seurat_object <-
  function(
    ...
    for (index in names(samples)) {
      ...
      meta_path <- file.path(input_dir, index, 'outs', 'per_sample_outs', sample, 'count', 'sample_filtered_barcodes.csv')
      meta_data <- read.csv(meta_path, header =  F, row.names = 2)
      colnames(meta_data) <- "Genome"

      # Create seurat object
      sobj <- Seurat::CreateSeuratObject(
        counts = rna_list[[sample]],
        project = project,
        min.cells = min.cell,
      ...
      sobj[['MouseID']] <- sample
      sobj[['SampleID']] <- index
      sobj <- Seurat::RenameCells(sobj, add.cell.id = paste0(index, "_", sample))
      ...
      if (save){
        save_name <- file.path(save_dir, "INDIVIDUAL_SeuratObjects", paste0(sample, "_rna.qs"))
        dir.create(file.path(save_dir, "INDIVIDUAL_SeuratObjects"), recursive = T)

        qs::qsave(sobj, file = save_name)
        message("Saved object: ", sample, " to ", save_name, ".")
      }
      }
    }
    total_samples <- sum(lengths(samples))
    message("Created ", length(sobj_list), " of ", total_samples, " Seurat objects.")
